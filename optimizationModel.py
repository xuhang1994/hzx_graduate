...
def extendedResult(result):
    ...
    ax1.bar(result.index.values.tolist(),sheet1['燃气轮机发电功率'],bottom=sheet1['购电功率'],color = '#42f486')
    ax1.bar(result.index.values.tolist(),sheet1['电储能放电功率'],bottom=sheet1['燃气轮机发电功率']+sheet1['购电功率'],color = '#f442ee')
    ax1.bar(result.index.values.tolist(), sheet1['光伏出力'], bottom=sheet1['燃气轮机发电功率'] + sheet1['购电功率']+sheet1['电储能放电功率'],
            color='#ff8000')
    ax1.bar(result.index.values.tolist(),sheet1['电储能充电功率'],color = '#41b8f4')
    ax1.bar(result.index.values.tolist(),sheet1['冰蓄冷耗电功率'],bottom=sheet1['电储能充电功率'],color = '#408080')
    ax1.bar(result.index.values.tolist(),sheet1['空调制冷耗电功率'],bottom=sheet1['冰蓄冷耗电功率']+sheet1['电储能充电功率'],color ='#7f41f4' )
    first_legend = plt.legend([load],('电负荷',))
    plt.gca().add_artist(first_legend)
    fig.legend([mpatches.Patch(color = c) for c in sheet1colors],['购电功率','燃气轮机发电功率','电储能放电功率','光伏出力','电储能充电功率','冰蓄冷耗电功率','空调制冷耗电功率'])
    ax1.set_xlabel('时间')
    ax1.set_ylabel('功率(kW)')
    ax2 = ax1.twinx()
    price,= ax2.plot(result.index.values.tolist(),sheet1['电价'],linewidth=3.0, linestyle='-', label='电价')
    second_legend = plt.legend([price], ('电价',))
    ax2.set_ylabel('电价(元)')
    ...
    sheet5.to_excel(writer, sheet_name='中品位热平衡')
    plt.figure(3)
    plt.rcParams['font.sans-serif'] = ['SimHei']
    load, = plt.plot(-sheet5['热负荷'], linewidth=3.0, linestyle='--', label='蒸汽热负荷')
    ...
    sheet6['热水负荷'] = -result['热水负荷']
    sheet6['电价'] = result['电价']
    sheet6.to_excel(writer, sheet_name='低品位热平衡')
    plt.figure(4)
    plt.rcParams['font.sans-serif'] = ['SimHei']
    load, = plt.plot(-sheet6['吸收式制冷机耗热功率'], linewidth=3.0, linestyle='--', label='吸收式制冷机耗热功率')
    load, = plt.plot(-sheet6['热水负荷'], linewidth=3.0, linestyle='-', label='热水负荷')
    sheet6colors = ['#f4f441', '#42f486']
    plt.bar(result.index.values.tolist(), sheet6['蒸汽回收低品位热'], color='#f4f441')
    plt.bar(result.index.values.tolist(), sheet5['余热锅炉低品位热功率'], bottom=sheet6['蒸汽回收低品位热'], color='#42f486')
    first_legend = plt.legend([load], ('吸收式制冷机耗热功率',))
    ax = plt.gca().add_artist(first_legend)
    plt.legend([mpatches.Patch(color=c) for c in sheet6colors], ['蒸汽回收低品位热功率','余热锅炉低品位热功率'])
    plt.xlabel('时间')
    plt.ylabel('功率(kW)')
    plt.show()
    return
...
def DayInModel(microgrid_data,case,nowtime,data,realdata):
    ...
    optimalDispatch.HPB6 = Constraint(T, rule=lambda mdl, t: mdl.low_heat[t] + mdl.medium_heat[t] >= steam_heat_load[t] + sum(mdl.absc_heat_in[n_absc, t] for n_absc in N_absc))
